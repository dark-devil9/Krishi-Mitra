...
def extract_location_from_query(query: str):
    ...
    # Look for Indian locations in the query with stopword safeguards
    stopword_like = {"such", "some", "any", "budget", "profit", "grow", "crop", "crops"}
    for location in indian_locations:
        if location in query_lower:
            # Avoid false positives like 'such' -> 'Suchindram'
            if location in stopword_like:
                continue
    ...
    for pattern in location_patterns:
        ...
        if match:
            ...
            # Avoid locations that look like generic words
            if location in stopword_like:
                continue
